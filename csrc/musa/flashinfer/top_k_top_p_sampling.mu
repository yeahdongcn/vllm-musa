...
// Vendored flashinfer sampling — at csrc/musa/flashinfer/sampling.muh
#include "sampling.muh"

// Minimal CHECK macros (replace pytorch_extension_utils.h)
#define CHECK_INPUT(x)                                                          \
  TORCH_CHECK(x.device().is_privateuseone(), #x " must be on a MUSA device");   \
  TORCH_CHECK(x.is_contiguous(), #x " must be contiguous")
#define CHECK_EQ(a, b) TORCH_CHECK((a) == (b), #a " == " #b " mismatch")
#define CHECK_DIM(d, x) TORCH_CHECK(x.dim() == (d), #x " must be " #d "-D")

namespace vllm_musa {
namespace sampling_port {
...
musaError_t VllmMusaTopKTopPSamplingFromProb(T* probs, IdType* top_k_arr, T* top_p_arr, IdType* output,
  ...
  DISPATCH_COMPUTE_CAP_NUM_THREADS(compute_capacity, BLOCK_THREADS, {
    ...
    if (d < BLOCK_THREADS * kRmemEles) {
      ...
      DISPATCH_ALIGNED_VEC_SIZE(
          vec_size, VEC_SIZE, {DISPATCH_DETERMINISTIC(deterministic, DETERMINISTIC, {
            auto kernel = vllm_musa::sampling_port::TopKTopPSamplingFromProbKernel<
                BLOCK_THREADS,
                SCAN_ALGO,
                REDUCE_ALGO,
                VEC_SIZE,
...
void musa_top_k_top_p_sampling_impl(
  ...
  musaError_t status = VllmMusaTopKTopPSamplingFromProb<float, int>(
  ...
  TORCH_CHECK(
      status == musaSuccess,
      "VllmMusaTopKTopPSamplingFromProb failed with error code " + std::string(musaGetErrorString(status)));
}
